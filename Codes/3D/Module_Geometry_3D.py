...
def carToSpherical(Nz=1, Ny=1, Nx=1, cz=0, cy=0, cx=0):
    x = np.arange(Nx)
    y = np.arange(Ny)
    z = np.arange(Nz)
    ...
    r = np.sqrt(Z_shifted**2 + Y_shifted**2 + X_shifted**2)
    phi = np.arctan2(Y_shifted, X_shifted)   
    theta = np.arctan2(np.sqrt(X_shifted**2 + Y_shifted**2), Z_shifted)
