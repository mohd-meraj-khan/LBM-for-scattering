...
fig = plt.figure(figsize = (3.35, 3.35), dpi=600)
gs=GridSpec(1,1)
plt.ion()
...
with writer.saving(fig, Video_Name,300):
    for t in range(int(Time)):
        ...
        if (t >= 0):
            ...
            myclib.collStreamForcingNode(ex, ey, ez, hx, hy, hz, Ex, Ey, Ez, Hx, Hy, Hz, er, mur, Nz, Ny, Nx, Q, xmin, xmax, ymin, ymax)

        else:
            
            # collision and streaming (the 2 steps of LBM) when field is not forced
            myclib.collStream(ex, ey, ez, hx, hy, hz, Ex, Ey, Ez, Hx, Hy, Hz, er, mur, Nz, Ny, Nx, Q)
