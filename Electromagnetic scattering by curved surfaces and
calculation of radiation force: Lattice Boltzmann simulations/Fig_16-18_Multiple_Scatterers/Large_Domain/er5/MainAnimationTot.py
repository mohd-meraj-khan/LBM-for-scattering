...
er1, mur1, er2 = 1, 1, 5   # material properties i.e. permittivity and permeabilty
...
with writer.saving(fig, Video_Name,300):
    for t in range(int(Time)):
        ...
        U[t] = np.sum(0.5 * (er1 * (Ex[0, Y1:Y2, X1:X2]**2 + Ey[0, Y1:Y2, X1:X2]**2 + Ez[0, Y1:Y2, X1:X2]**2) + mur1 * (Hx[0, Y1:Y2, X1:X2]**2 + Hy[0, Y1:Y2, X1:X2]**2 + Hz[0, Y1:Y2, X1:X2]**2)))
    ...
    ###############################################################################################################
        ...
        if (t%10 == 0):
            ...
            im = plt.imshow(er[0], extent=(0, Nx, 0, Ny), cmap='binary', origin='lower', alpha=0.25)


            plt.axhline(y = Ny//3-1, linestyle='--', color='black')
            plt.axhline(y = 2*Ny//3-1, linestyle='--', color='black')

            plt.axvline(x = Nx//3-1, linestyle='--', color='black')
            plt.axvline(x = 2*Nx//3-1, linestyle='--', color='black')

            
            ax1.set_xticks(np.linspace(0,Nx,4))
            ax1.set_xticklabels([0, nx, 2*nx, 3*nx])
            ax1.set_xlabel(r'$x/a$', fontsize=12)
            ax1.set_yticks(np.linspace(0,Ny,4))
            ax1.set_yticklabels([0, ny, 2*ny, 3*ny])
