...
void collForcingNode(float* ex, float* ey, float* ez, float* hx, float* hy, float* hz, float* exb, float* eyb, float* ezb, float* hxb, float* hyb, float* hzb, float* Ex, float* Ey, float* Ez, float* Hx, float* Hy, float* Hz, float* er, float* mur, int z, int y, int x, int q, int xloc, int ymin, int ymax) {
	...
	for (i = 0; i < z; i++) {
		...
		for (j = 0; j < y; j++) {
			for (k = 0; k < x; k++) {
				...
				if (j >= ymin && j < ymax && k == xloc) { // no collision at the forcing node
