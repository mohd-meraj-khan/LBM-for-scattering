...
def carToPolar(r, phi, Nz=1, Ny=10, Nx=10, cy=0, cx=0):
    for i in range(Ny):
        for j in range(Nx):
...
def circle(r, a, Nz=1, Ny=10, Nx=10):
...
def square(a, Nz=1, Ny=10, Nx=10, cy=0, cx=0):
    inside = np.zeros((Nz, Ny, Nx), dtype=bool)
    for i in range(Ny):
        for j in range(Nx):
            Y = i - cy
...
def hexagon(a, Nz=1, Ny=10, Nx=10, cy=0, cx=0):
    inside = np.zeros((Nz, Ny, Nx), dtype=bool)
    for i in range(Ny):
        for j in range(Nx):
            Y = i - cy
...
def corrugatedEllipse(r, phi, a, Nz=1, Ny=10, Nx=10, A=1, N=0, epsilon=0):
    inside = np.zeros((Nz, Ny, Nx), dtype=bool)
    for i in range(Ny):
        for j in range(Nx):
            if (r[0, i, j] <= a * (1 / np.sqrt((np.cos(phi[0, i, j]))**2 + (A * np.sin(phi[0, i, j]))**2) + epsilon * np.cos(N * phi[0, i, j]))):
                inside[0, i, j] = True
    return inside
#################################################################
...
def JanusHalf(r, er, a, theta, er2, er3, Nz=1, Ny=10, Nx=10, cy=0, cx=0):
    for i in range(Ny):
        for j in range(Nx):
            Y = i - cy
            X = j - cx
...
def JanusSegment(r, er, a, er2, er3, Nz=1, Ny=10, Nx=10, cy=0, cx=0, shift=0):
    for i in range(Ny):
        for j in range(Nx):
            Y = i - cy
            X = j - cx
...
def JanusSector(r, er, a, er2, er3, Nz=1, Ny=10, Nx=10, cy=0, cx=0, theta=0):
    for i in range(Ny):
        for j in range(Nx):
            Y = i - cy
            X = j - cx
