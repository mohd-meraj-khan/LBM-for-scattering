...
a, ratio = 50, 2   # ratio = a / wavelength
n = 10
...
# scatterer particle
scatterer = ModuleGeometry.hexagon(a, Nz, Ny, Nx, cy, cx)
...
fig = plt.figure(figsize = (8.0139, 6.2739), dpi=600)
gs=GridSpec(1,1)
plt.ion()
...
with writer.saving(fig, Video_Name,300):
    ...
    ###############################################################################################################
        ...
        if (t - int(Time) >= -3):
            ...
            plt.rc('lines', markersize = 2, lw = 1.5)
...
##            if (t - Time == -1):
            plt.savefig(pictures+"/pic."+str(t).zfill(4)+".eps")
##            writer.grab_frame()     
