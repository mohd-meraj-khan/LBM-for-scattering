...
from ModuleSourceOfEmWave import planeWaveTM, planeWaveTE
...
a, ratio = 200, 2   # ratio = a / wavelength
n = 8
...
# scatterer particle
A, N, epsilon = 0.5, 10, 0.1

scatterer = ModuleGeometry.corrugatedEllipse(r, phi, a, Nz, Ny, Nx, A, N, epsilon)
...
with writer.saving(fig, Video_Name,300):
    for t in range(int(Time)):
        ...
        if (t >= 0):
            ...
            # source wave
            planeWaveTE(Ey, Hz, t, omega, xloc, ymin, ymax)
    ...
    ###############################################################################################################
        ...
        if (t%10 == 0):
            ...
            plt.title(r'$H_z^{tot}$')
            im1 = plt.imshow(Hz[0], vmin = -1, vmax = 1, cmap='seismic', origin='lower')
    ...
    ###############################################################################################################
        ...
        if (t - int(Time) >= -1):        
            field = open(data+"/Hz_{}.txt".format(A), "w")
            np.savetxt(field, Hz[0,:,:])
            field.close()
