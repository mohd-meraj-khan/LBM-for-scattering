...
for t in range(int(Time)):
    ...
    if (t >= int(Time) - int(np.round(period))):
        ...
        TdotN = tractionFx(Ex_interp, Ey_interp, Ez_interp, Hx_interp, Hy_interp, Hz_interp, er1, mur1, nx, ny, nz)
        Traction.append(TdotN)
        Fx.append(np.sum(TdotN * R * dtheta * (np.pi / 180) / (a / ratio)))
        ###############################################################################################################
...
Fx_avg.append(np.average(Fx))
print(Fx_avg[0], '\n')




fxAvg = open(directory+"/Fx_Avg_ratio_{}.txt".format(ratio), "a")
np.savetxt(fxAvg, Fx_avg, fmt='%.4e')
fxAvg.close()

fxSum = open(directory+"/Fx_Sum_ratio_{}_a_{}.txt".format(ratio, a), "w")
np.savetxt(fxSum, Fx, fmt='%.4e')
fxSum.close()

traction = open(directory+"/traction_ratio_{}_a_{}.txt".format(ratio, a), "w")
np.savetxt(traction, Traction, fmt='%.4e')
traction.close()
