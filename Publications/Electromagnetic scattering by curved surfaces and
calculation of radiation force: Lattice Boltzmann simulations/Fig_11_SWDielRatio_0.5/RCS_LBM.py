...
for j in range(len(r)):
    ...
    Escat = np.loadtxt(directory+'/Ez_scat{}_{}.txt'.format(r[j], er2))
    ...
    brcs = open(directory+"/BRCS_LBM{}_{}.txt".format(r[j], er2), "w")
    np.savetxt(brcs, RCS_LBM)
    brcs.close()

    MRCS = np.array([RCS_LBM[180]])

    mrcs = open(directory+"/MRCS_LBM{}_{}.txt".format(r[j], er2), "w")
    ...
    plt.savefig(directory1+'/EzS_ratio_{}_r_{}.svg'.format(er2, r[j]))
    plt.close()
    ################################################################################
